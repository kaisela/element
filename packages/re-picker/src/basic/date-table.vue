...
<script>
  ...
  import {
    hasClass
  } from 'my-element-ui/src/utils/dom';
    ...
    methods: {
      ...
      calcId(cell) {
        if (this.getCellClasses(cell).indexOf('prev-month') > -1) {
          let monthTemp = (this.month + 1) < 11 ? ('0' + (this.month)) : (this.month);
          let year = this.year;
          if (monthTemp === '00') {
            monthTemp = '12';
            year--;
          }
          return year + '-' + monthTemp + '-' + (cell.text < 10 ? ('0' + cell.text) : cell.text);
        } else if (this.getCellClasses(cell).indexOf('next-month') > -1) {
          let monthTemp = (this.month + 1) < 9 ? ('0' + (this.month + 2)) : (this.month + 2);
          let year = this.year;
          if (monthTemp === '13') {
            monthTemp = '01';
            year++;
          ...
          return this.year + '-' + ((this.month + 1) < 10 ? ('0' + (this.month + 1)) : (this.month + 1)) + '-' + (cell.text < 10 ? ('0' + cell.text) : cell.text);
        }
      },
      getCellClasses(cell) {
      ...
      handleClick(event) {
        ...
        if (target.tagName !== 'TD' && target.tagName !== 'SPAN') return;
        if (hasClass(target, 'disabled') || hasClass(target, 'week') || hasClass(target, 'today')) return;
